...
class Calendar
  ...
  def initialize(year, month)
    #@fst_date = fst_day_in_week(year, month)
    #@last_date = Date.new(year, month, get_last_day(year, month))
  ...
  def fst_day_in_week(year, month)
  ...
  if num == 1
    ...
    7.times do
      4.times do
        #puts i
        print "#{(@fst_date += i).strftime('%d').to_i}  i = #{i} "
      end
      puts "\n"
    i += 1
    end
    ...
    print "\n"
    5.times do 
      print (@fst_date += 8).strftime('%d').to_i
    end
  ...
  def dates_in_month(year, month)
    ...
    @fst_date = Date.new(year, month, 1)
    @last_date = Date.new(year, month, get_last_day(year, month))
    @arr_dates = (@fst_date..@last_date).to_a
  end
